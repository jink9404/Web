...
public class CommandReply implements Command{
	private String next;
	public CommandReply(String next) {
		this.next = next;	
	}
	
	public String execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
			try {
			BoardRec c_rec = new BoardRec();
			c_rec.setTitle(request.getParameter("title"));
			c_rec.setWriterName(request.getParameter("writerName"));
			c_rec.setContent(request.getParameter("content"));
			c_rec.setPassword(request.getParameter("password"));
			String p_rec_id = request.getParameter("parentId");
			BoardRec r_rec = reply(p_rec_id, c_rec);
			request.setAttribute("rec", r_rec);
			}catch (BoardException e) {
				throw new CommandException("CommandReply error "+e.toString());
			}
		return next;
	}
